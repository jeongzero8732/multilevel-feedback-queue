...
void ExecuteProcess()
{
	...
	if(q[index].front->data.execut_time<q[index].time_quantum) //시간 할당량 보다 수행한 시간이 작을 때
	{
		q[index].front->data.remain_time--;//프로세스 남은 시간 --
		q[index].front->data.execut_time++;//프로세스 수행한 횟수++

		if(q[index].front->data.remain_time==0) //프로세스가 끝났으면 종료
		{
			p_temp=*(Dequeque(q));; // 큐에 저장되어 있는 종료한 프로세스 삭제 한 후
			CPU_finish(&p_temp); // 출력 함수
		}
		else if(q[index].front->data.remain_time!=0) //수행할 시간이 남았다면
		{
			index_temp2=index+1;
	...
	else // 시간 할당량 보다 수행한 시간이 같거나 클 때 강등시키거나 라운드로빈으로 스케줄링
	{
				...
				else 
				{
					...
					{	
						if(index_temp==8)
						{
							...
							else if(q[index].front->next!=NULL)
							{
								q[index].front->data.execut_time=0;// 실행한 시간 초기화
								p_temp=*(Dequeque(q));; //해당 노드 디큐
								Degrade(&p_temp);//저장해뒀던 노드를 강등
